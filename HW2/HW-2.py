...
with open('./hw2-files.txt') as f:
    ...
    text_RDD = sc.textFile(file_path[0]).cache()
...
"""
interm = text_RDD.flatMap(safe_parse)
pair_RDD = interm.map(lambda x: (x["user"]["id_str"], x["text"])) 
pair_RDD.cache()
...
# YOUR CODE HERE
users_count = len(pair_RDD.countByKey())
...
groups_rdd = sc.parallelize(partition.items())
pair_RDD_counts = pair_RDD.map(lambda x: (x[0],1))
intersection = pair_RDD_counts.cogroup(groups_rdd)
intersection = intersection.map(lambda x: (x[0],list(x[1][1]), sum(x[1][0])))
intersection = intersection.map(group7).cache()
counts_per_partition = intersection.map(lambda x: (x[1],x[2]))
counts_per_partition = counts_per_partition.reduceByKey(lambda x,y: x+y).sortByKey(ascending=True)
counts_per_partition = counts_per_partition.collect()
...
res = pair_RDD.flatMapValues(tok.tokenize)

res = res.map(lambda x: (x,1))
res = res.reduceByKey(lambda x,y: 1) #Getting rid of same tweets by the same user
interm = res.map(lambda x: x[0])
intersection = intersection.map(lambda x: (x[0],x[1]))
interm = interm.join(intersection)
interm = interm.map(lambda x: ((x[0],x[1][1]),x[1][0]))
interm.cache()
res = res.map(lambda x: (x[0][1],1))
res = res.reduceByKey(lambda x,y: x+y)
res.cache()
num_of_tokens = res.count()
...
# YOUR CODE HERE
res = res.filter(lambda x: x[1] >= 100)
num_freq_tokens = res.count()
top20 = res.takeOrdered(20,key = lambda x: -x[1])
...
# YOUR CODE HERE
res2 = interm
res2 = res2.map(lambda x: (x[1],x[0]))
res2 = res.join(res2)
res2 = res2.map(lambda x: ((x[1][1][1],x[0],x[1][0]),1))
res2 = res2.reduceByKey(lambda x,y: x+y)
res2 = res2.map(lambda x: (x[0][0],(x[0][1],x[0][2],x[1])))
res_repartitioned = res2.partitionBy(8,lambda x: x).map(lambda x: ((x[1][0],get_rel_popularity(x[1][2], x[1][1]))))
res_repartitioned= res_repartitioned.mapPartitions(lambda p: sorted(p, key = lambda x: (-x[1],x[0])))
a = res_repartitioned.glom().collect()

popular_10_in_each_group = []
for n in range(8):
    popular_10_in_each_group.append(a[n][0:10])
...
print(timer)
print("Total time: ", sum([x[1] for x in timer]))
